...
\outer\def\section #1.{\penalty-50\vskip 12pt plus 3pt minus 3pt
  \noindent\vbox to 0pt{\kern-2.5ex\relax
    \ifacro\ifpdftex\pdfdest num\destcount fitbh\relax
    \else\special{pdf: dest (\destcount) [ @thispage /FitBH @ypos]}\fi\fi
    \def\.##1{##1}%
    \ifx\bkminfo\empty
      \ifacro\ifpdftex\pdfoutline goto num\destcount{#1}\relax
      \else\special{pdf: outline 0 << /Title (#1)
        /A << /S /Goto /D (\destcount) >> >>}\fi\fi
    \else
      \ifacro\ifpdftex\pdfoutline goto num\destcount{#1 (\bkminfo)}\relax
      \else\special{pdf: outline 0 << /Title (#1 (\bkminfo))
        /A << /S /Goto /D (\destcount) >> >>}\fi\fi
    \fi
    \gdef\bkminfo{}%
